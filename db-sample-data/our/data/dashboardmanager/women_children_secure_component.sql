...
VALUES (
        ...
        'two_d',
        'WITH district_list AS (     SELECT unnest(ARRAY[         ''北投區'',''士林區'',''內湖區'',''南港區'',''松山區'',''信義區'',         ''中山區'',''大同區'',''中正區'',''萬華區'',''大安區'',''文山區''     ]) AS district )  SELECT     d.district AS x_axis,     COALESCE(c.data, 0) AS data FROM district_list d LEFT JOIN LATERAL (         SELECT COUNT(*) AS data         FROM   women_children_secure w         WHERE  w.location LIKE ''%'' || d.district || ''%''     ) c ON TRUE ORDER BY     array_position(         ARRAY[             ''北投區'',''士林區'',''內湖區'',''南港區'',''松山區'',''信義區'',             ''中山區'',''大同區'',''中正區'',''萬華區'',''大安區'',''文山區''         ],         d.district     ); ',
        NULL,
        'taipei'
    ),
    (
        ...
        'two_d',
        'WITH district_list(district, ord) AS (     VALUES       (''北投區'',  1), (''士林區'',  2), (''內湖區'',  3), (''南港區'',  4),       (''松山區'',  5), (''信義區'',  6), (''中山區'',  7), (''大同區'',  8),       (''中正區'',  9), (''萬華區'', 10), (''大安區'', 11), (''文山區'', 12),       (''新莊區'', 13), (''淡水區'', 14), (''汐止區'', 15), (''板橋區'', 16),       (''三重區'', 17), (''樹林區'', 18), (''土城區'', 19), (''蘆洲區'', 20),       (''中和區'', 21), (''永和區'', 22), (''新店區'', 23), (''鶯歌區'', 24),       (''三峽區'', 25), (''瑞芳區'', 26), (''五股區'', 27), (''泰山區'', 28),       (''林口區'', 29), (''深坑區'', 30), (''石碇區'', 31), (''坪林區'', 32),       (''三芝區'', 33), (''石門區'', 34), (''八里區'', 35), (''平溪區'', 36),       (''雙溪區'', 37), (''貢寮區'', 38), (''金山區'', 39), (''萬里區'', 40),       (''烏來區'', 41) )  SELECT     d.district AS x_axis,     COALESCE(         ( SELECT COUNT(*)           FROM   women_children_secure w           WHERE  w.location LIKE ''%'' || d.district || ''%'' ),         0     ) AS data FROM   district_list d ORDER  BY d.ord; ',
        NULL,
        'metrotaipei'
    );