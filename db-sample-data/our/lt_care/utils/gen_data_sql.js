...
function csvToSql(csvFilePath, outputSqlPath, tableName = 'elder_care_location') {
    try {
        // 讀取 CSV 文件
...
`;
        ...
        // 生成 INSERT 語句
        sqlStatements.push(`\n-- 插入資料`);
        sqlStatements.push(`INSERT INTO ${tableName} (hosp_name, hosp_addr, phone, district, longitude, latitude, ratio) VALUES`);
...
function main() {
    // 設定文件路徑
    const csvFile = path.join(__dirname, '..', 'data', 'lt_care_metro_latlng.csv');
    const outputFile = path.join(__dirname, '..', '..', 'data', 'dashboard', 'elder_care_location.sql');
